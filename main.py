            ...
            if branch.name == target_branch:
                ...
                if branch.commit.hexsha == last_commit:
                    ...
                    if f'Merge branch \'{TARGET_BRANCH}\' of' not in branch.commit.message:
                        ...
                        print(f'Latest commit in repo {path_to_src_repo} is "Merge" (\'git pull\' command). I won\'t do anything.')  
...
if __name__ == '__main__':
    schedule.every(10).seconds.do(get_last_commit, path_to_src_repo=PATH_TO_DEV_REPO, 
                path_to_dest_repo=PATH_TO_PROD_REPO, target_branch=ORIGINAL_REMOTE_REPO  + '/' + TARGET_BRANCH, 
                original_name=DEV_REPO_NAME, original_email=DEV_REPO_EMAIL, 
                new_name=NEW_NAME, new_email=NEW_EMAIL, commit_file='last_commit_dev.txt')
    schedule.every(15).seconds.do(get_last_commit, path_to_src_repo=PATH_TO_PROD_REPO, 
                path_to_dest_repo=PATH_TO_DEV_REPO, target_branch=ORIGINAL_REMOTE_REPO  + '/' + TARGET_BRANCH, 
                original_name=PROD_REPO_NAME, original_email=PROD_REPO_EMAIL, 
                new_name=NEW_NAME, new_email=NEW_EMAIL, commit_file='last_commit_prod.txt')
    while True:
